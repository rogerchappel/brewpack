...
export function packageJsonToFixture(raw) {
    ensureObject(raw, 'package.json');
    const bin = raw.bin;
    const binaries = typeof bin === 'string'
        ? [String(raw.name ?? '')]
...
export function parsePackageFixture(raw) {
    ...
    if (!slug)
        throw new Error('fixture.package.name must contain at least one alphanumeric character.');
    const homepage = typeof pkg.homepage === 'string' && pkg.homepage.trim() ? pkg.homepage.trim() : `https://example.com/${slug}`;
    const repository = normalizeRepository(pkg.repository, homepage);
    ...
    if (testInput !== undefined)
        ensureObject(testInput, 'fixture.package.test');
...
export function validateTapLayout(entries, formulaEntries = []) {
    const missing = [];
    const warnings = [];
    if (!entries.includes('Formula'))
